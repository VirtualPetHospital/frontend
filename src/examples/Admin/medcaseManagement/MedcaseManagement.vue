      ...
      >
        ...
        <el-form :model="form" :rules="rules" ref="form" label-width="120px">
          ...
          <el-form-item label="疾病名" prop="diseaseName">
            <el-select v-model="form.diseaseName" placeholder="请选择疾病名">
              <el-option
                v-for="item in diseaseNames"
                :key="item.value"
                :label="item.text"
                :value="item.value">
              </el-option>
          ...
          <el-form-item label="价格" prop="price">
            <el-input v-model="form.price" placeholder="请输入价格" type="number" :min="0" :max="99999.99" step="0.01"></el-input>
          </el-form-item>
          <el-form-item label="诊断结果" prop="diagnosisResult">
            <el-input v-model="form.diagnosisResult" placeholder="请输入诊断结果"></el-input>
          </el-form-item>
          <el-form-item label="病例名" prop="caseName">
            <el-input v-model="form.caseName" placeholder="请输入病例名"></el-input>
          </el-form-item>
          <el-form-item label="接诊图片" prop="caseImage">
            <el-upload
              class="upload-demo"
              action="https://jsonplaceholder.typicode.com/posts/"
              :on-preview="handlePreview"
              :on-remove="handleRemove"
              :file-list="form.caseImage"
              list-type="picture-card">
              <i class="el-icon-plus"></i>
            </el-upload>
          </el-form-item>
          <el-form-item label="接诊视频" prop="caseVideo">
            <el-upload
              class="upload-demo"
              action="https://jsonplaceholder.typicode.com/posts/"
              :on-preview="handlePreview"
              :on-remove="handleRemove"
              :file-list="form.caseVideo"
              list-type="picture-card">
                ...
                <div class="col-12">
                  <div class="user-management-container">
                    ...
                    >
                      ...
                      <el-table-column prop="caseName" label="病例名"></el-table-column>
                      <el-table-column prop="caseImage" label="接诊图片"></el-table-column>
                      <el-table-column prop="caseVideo" label="接诊视频"></el-table-column>
                    </el-table>
                  </div>
                </div>
              </div>
  ...
  export default {
    ...
    return {
      ...
      rules: {
        ...
        diseaseName: [{ required: true, message: '请选择疾病名', trigger: 'blur' }],
        price: [
          { required: true, message: '请输入价格', trigger: 'blur' },
          { pattern: /^(0|[1-9]\d{0,4})(\.\d{1,2})?$/, message: '请输入正确的价格', trigger: 'blur' }
        ],
        diagnosisResult: [{ required: true, message: '请输入诊断结果', trigger: 'blur' }],
        caseName: [{ required: true, message: '请输入病例名', trigger: 'blur' }],
        // caseImage: [{ required: true, message: '请上传接诊图片', trigger: 'blur' }],
        // caseVideo: [{ required: true, message: '请上传接诊视频', trigger: 'blur' }]

      },
      diseaseTypes: [
        { value: 'type1', text: '病种1' },
        { value: 'type2', text: '病种2' },
        { value: 'type3', text: '病种3' }
      ],
      diseaseNames: [
        { value: 'name1', text: '疾病1' },
        { value: 'name2', text: '疾病2' },
        { value: 'name3', text: '疾病3' }
      ],
      cases: [
  { 
    id: 1, 
    ...
    diseaseName: "疾病1", 
    price: 100, 
    diagnosisResult: "诊断结果1", 
    caseName: "病例1", 
    caseImage: [{ name: "image1.jpg", uid: "-1" }], // 添加 uid 属性
    caseVideo: [{ name: "video1.mp4", uid: "-2" }] // 添加 uid 属性
  },
  { 
    id: 2, 
    diseaseType: "病种2", 
    diseaseName: "疾病2", 
    price: 200, 
    diagnosisResult: "诊断结果2", 
    caseName: "病例2", 
    caseImage: [{ name: "image2.jpg", uid: "-3" }], // 添加 uid 属性
    caseVideo: [{ name: "video2.mp4", uid: "-4" }] // 添加 uid 属性
  },
  { 
    id: 3, 
    diseaseType: "病种3", 
    diseaseName: "疾病3", 
    price: 300, 
    diagnosisResult: "诊断结果3", 
    caseName: "病例3", 
    caseImage: [{ name: "image3.jpg", uid: "-5" }], // 添加 uid 属性
    caseVideo: [{ name: "video3.mp4", uid: "-6" }] // 添加 uid 属性
  },
  // 其他病例数据...
],
      currentPage: 1, // 当前页码
  ...
  computed: {
    ...
    filteredCases() {
      const filtered = this.cases.filter(item =>
        item.caseName.toLowerCase().includes(this.searchText.toLowerCase())
      );
      return filtered;
    },
  },
  methods: {
    ...
    openModifyDialog() {
      ...
      if (this.selectedRow) {
        ...
        this.form.caseName = this.selectedRow.caseName;
        this.form.caseImage = this.selectedRow.caseImage;
        this.form.caseVideo = this.selectedRow.caseVideo;
